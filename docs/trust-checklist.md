...
| 7 | ENOSPC non-destructive + alerted | ✅ | Chaos #4: `TestChaos04_ENOSPC` — tiny FS (darwin ram disk / Linux tmpfs), fill near full, write fails with ENOSPC-class error, failure alert fires, **0 partial snapshots** after reopen. Mount is outside `t.TempDir` with verified umount (CHAOS-F3 — Linux CI no longer flakes on cleanup EBUSY). |
