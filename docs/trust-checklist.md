...
| 2 | Restore byte+ACL+ADS+timestamp identical | ⚠️ M4 portable bytes+symlinks green; ACL/ADS Windows-untested |
| 3 | Cross-machine restore works | ✅ M4 (`TestM4_CrossMachineRestore`) |
...
| 12 | Server-loss drill: metadata rebuilt from repo dir alone | ⚠️ M4 partial (`TestM4_ServerLossDrill` wipe index+rescan+restore); full container drill still M6 |
